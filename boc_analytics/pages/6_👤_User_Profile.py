...
st.set_page_config(page_title="BOC · User Profile", page_icon="👤", layout="wide")
st.markdown("""
<style>
...
with st.sidebar:
    ...
    search_term = st.text_input("Name or Email:", placeholder="e.g. Alice, gmail.com")

    if search_term:
        matches = user[
            user["name"].str.contains(search_term, case=False, na=False) |
            user["email"].str.contains(search_term, case=False, na=False)
        ]
    else:
        matches = user.head(50)

    user_options = matches[["id","name","email"]].copy()
    user_options["label"] = user_options["name"] + " — " + user_options["email"]

    if len(user_options) == 0:
        st.warning("No users found.")
        st.stop()

    # Default to user with most bills (not Ganesh)
    if not bill.empty and "userId" in bill.columns and not search_term:
        _bill_counts = bill.groupby("userId").size().reset_index(name="_cnt")
        _user_with_bills = user.merge(_bill_counts, left_on="id", right_on="userId", how="left")
        _user_with_bills["_cnt"] = _user_with_bills["_cnt"].fillna(0)
        _default_user = _user_with_bills.sort_values("_cnt", ascending=False).head(50)
        user_options = _default_user[["id","name","email"]].copy()
        user_options["label"] = user_options["name"] + " — " + user_options["email"]

    sel_label = st.selectbox("Select user:", user_options["label"].tolist())
...
with r2:
    # Try to get categories from the bill_extraction table directly
    # (the left-join may have all-null be columns if billIds don't match)
    _cat_data = pd.Series(dtype=str)
    if not u_ext.empty and "category" in u_ext.columns:
        _cat_data = u_ext["category"].dropna()
    # Fallback: direct lookup in be using the user's bill IDs
    if len(_cat_data) == 0 and not u_bills.empty and not be.empty and "billId" in be.columns:
        user_bill_ids = u_bills["id"].tolist()
        _be_direct = be[be["billId"].isin(user_bill_ids)]
        if "category" in _be_direct.columns:
            _cat_data = _be_direct["category"].dropna()

    if len(_cat_data) > 0:
        cat_cnt = _cat_data.value_counts().reset_index()
        cat_cnt.columns = ["category", "count"]
        fig = px.pie(
            cat_cnt, values="count", names="category",
